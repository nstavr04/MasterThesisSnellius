...
class DoubleMixConv(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, mid_channels=None, kernels_per_layer=1):
        ...
        self.mix1 = nn.Sequential(
            # depthwise-mix: must split exactly in_channels
        ...
        # ─── Stage 2: MixConv on mid_channels → 1×1 to out_channels ───
...
# Double depthwise separable convs
...
class MixDownDS(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernels_per_layer=1):
        ...
        self.maxpool_conv = nn.Sequential(
            nn.MaxPool2d(2),
            DoubleMixConv(in_channels, out_channels, kernels_per_layer=kernels_per_layer),
        )
...
class MixUpDS(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, bilinear=True, kernels_per_layer=1):
        ...
        if bilinear:
            ...
            self.up = nn.ConvTranspose2d(in_channels, in_channels // 2, kernel_size=2, stride=2)
            self.conv = DoubleMixConv(in_channels, out_channels, kernels_per_layer=kernels_per_layer)
